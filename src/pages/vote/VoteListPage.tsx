...
// TODO(contract):
// 온체인 factory는 `getElection(electionId)` 중심이라 목록 pagination이 바로 되지 않습니다.
// 이 화면은 indexer/backend가 관리하는 electionId 목록을 바탕으로
// `getElectionAddress` -> `getElectionSnapshot` -> `getElectionVoterSnapshot` 순서로 hydrate 하는 방식 권장.
      ...
      <div className="px-3 pt-3 pb-[14px]">
        ...
        <div className="flex items-center justify-between">
          ...
          {vote.badge === "end" ? (
            ...
            >
              {/* TODO(contract): 공개 투표는 `submitOpenVote`, 비공개 투표는 `submitEncryptedVote` 연결 */}
              투표하기
            </button>
          )}
        </div>
  ...
  return (
    <>
      ...
      <div className="px-5 flex flex-col gap-[10px] pb-4">
        {/* TODO(contract): 카드 데이터는 `getElectionConfig`, `getElectionState`, `getTotalVotesForCandidate` 결과를 매핑 */}
        {VOTE_ITEMS.map((item) => (
